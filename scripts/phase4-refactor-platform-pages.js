...
async function refactorPlatformPages() {
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      // Replace tools array with import and dynamic fetch
      if (content.includes('const tools =')) {
        content = content.replace(
          /const\s+tools\s*=\s*\[([\s\S]*?)\]\s*;?/m,
          `const toolsData = await import('@/data/strapi-mock/platforms/${platform}-tools.json');\nconst tools = toolsData.tools || [];`
        )
        modified = true
        console.log(`  ✓ Refactored tools array`)
      }

      // Replace strategy array with import and dynamic fetch
      if (content.includes('const strategy =')) {
        content = content.replace(
          /const\s+strategy\s*=\s*\[([\s\S]*?)\]\s*;?/m,
          `const strategyData = await import('@/data/strapi-mock/platforms/${platform}-strategy.json');\nconst strategy = strategyData.strategy || [];`
        )
        modified = true
...
async function main() {
  try {
    ...
    if (results.errors.length > 0) {
      console.log('\nErrors:')
      results.errors.forEach(e => console.log(`  ✗ ${e.platform}: ${e.error}`))
    }

    // Create summary report
    const reportPath = path.join(projectRoot, 'data', 'phase4-refactor-report.md')
