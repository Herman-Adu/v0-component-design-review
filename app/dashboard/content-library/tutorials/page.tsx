...
import { tutorials, type TutorialLevel, type TutorialCategory } from "@/data/content-library/tutorials"
...
export default function TutorialsPage() {
  ...
  const filteredTutorials = tutorials.filter((tutorial) => {
    ...
    const categoryMatch = categoryFilter === "all" || tutorial.category === categoryFilter
    return levelMatch && categoryMatch
  })

  const beginnerTutorials = filteredTutorials.filter((t) => t.level === "beginner")
  const intermediateTutorials = filteredTutorials.filter((t) => t.level === "intermediate")
  const advancedTutorials = filteredTutorials.filter((t) => t.level === "advanced")
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-lg text-muted-foreground text-balance">
          Hands-on, step-by-step guides covering Next.js, Strapi CMS, testing, and integrations.
          Each tutorial includes challenges with hidden solutions to test your understanding. Progress
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-green-500/5 border border-green-500/20 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-green-500">{tutorials.filter((t) => t.level === "beginner").length}</p>
        ...
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-yellow-500">{tutorials.filter((t) => t.level === "intermediate").length}</p>
        ...
        <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-4 text-center">
          <p className="text-2xl font-bold text-red-500">{tutorials.filter((t) => t.level === "advanced").length}</p>
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-card/50 border border-border rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(["all", "getting-started", "components", "forms", "security", "state-management", "performance", "cms", "testing"] as const).map((cat) => (
            <button
              key={cat}
      ...
      {beginnerTutorials.length > 0 && (
        <section className="space-y-4">
          ...
          <p className="text-muted-foreground">Start here to learn the fundamentals step by step.</p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 text-xs font-medium rounded ${getCategoryColor(tutorial.category)}`}>
      ...
      {intermediateTutorials.length > 0 && (
        <section className="space-y-4">
          ...
          <p className="text-muted-foreground">Build on your foundations with more complex patterns.</p>
          <div className="grid gap-4">
      ...
      {advancedTutorials.length > 0 && (
        <section className="space-y-4">
          ...
          <p className="text-muted-foreground">Master complex patterns used in production applications.</p>
          <div className="grid gap-4">
