...
const richContentMap: Record<string, ComponentType> = {
  "security-architecture": SecurityArchitectureContent,
  "deployment-guide": DeploymentGuideContent,
    ...
    <div className="space-y-8">
      ...
      <header className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`px-3 py-1 text-xs font-medium rounded-full border ${getLevelColor(guide.level)}`}>
            {guide.level.charAt(0).toUpperCase() + guide.level.slice(1)}
          </span>
          <span className={`px-3 py-1 text-xs font-medium rounded-full ${getCategoryColor(guide.category)}`}>
            {guide.category.charAt(0).toUpperCase() + guide.category.slice(1)}
          </span>
        </div>

        <h1 className="text-4xl font-bold text-foreground text-balance">{guide.title}</h1>
        <p className="text-xl text-muted-foreground text-pretty">{guide.description}</p>
        ...
        <div className="border-t border-border pt-8 space-y-6">
          ...
          {guide.sections.map((section, idx) => (
            <div key={section.id} className="bg-card border border-border rounded-lg p-6">
              ...
              <p className="text-sm text-muted-foreground mt-2">{section.summary}</p>
            </div>
          ))}
        </div>
      )}
