    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-lg text-muted-foreground text-balance">
          Real-world before-and-after transformations with measurable results. Each case study includes
          the problem context, solution approach, implementation details, and quantified outcomes.
        </p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {(["performance", "security", "architecture", "refactoring", "business", "cms"] as const).map((cat) => {
          ...
          return (
            ...
            >
              <p className={`text-2xl font-bold ${getCategoryColor(cat).split(" ")[1]}`}>
                {count}
              </p>
              <p className="text-sm text-muted-foreground capitalize">{cat === "cms" ? "CMS" : cat}</p>
            </button>
          )
        })}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-card/50 border border-border rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(["all", "performance", "security", "architecture", "refactoring", "business", "cms"] as const).map((cat) => (
            <button
              key={cat}
            ...
            >
              {cat === "all" ? "All" : cat === "cms" ? "CMS" : cat.charAt(0).toUpperCase() + cat.slice(1)}
            </button>
          ))}
        </div>
      </div>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                  <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded ${getCategoryColor(caseStudy.category)}`}>
                    {caseStudy.category}
                  </span>
              ...
              <div className="flex flex-wrap gap-4">
                {caseStudy.results.metrics.slice(0, 3).map((metric, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-green-500">{metric.improvement}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {filteredCaseStudies.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted-foreground">No case studies match your filter.</p>
        </div>
      )}
    </div>
  )
