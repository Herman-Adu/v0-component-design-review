...
export default function SocialMediaPage() {
  ...
  const [platform, setPlatform] = useState<"all" | "LinkedIn" | "Twitter/X" | "Facebook">("all")
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-4xl font-bold text-foreground mb-4">Social Media Posts</h1>
        <p className="text-xl text-muted-foreground">
          Ready-to-post content for LinkedIn, Twitter/X, and Facebook. Copy, customize, and share your learnings.
        </p>
      </div>
            ...
            {["all", "beginner", "intermediate", "advanced"].map((level) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  filter === level ? "bg-accent text-accent-foreground" : "bg-card text-muted-foreground hover:bg-muted"
                }`}
              >
          ...
          <div className="flex gap-2">
            {["all", "LinkedIn", "Twitter/X", "Facebook"].map((p) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  platform === p ? "bg-accent text-accent-foreground" : "bg-card text-muted-foreground hover:bg-muted"
                }`}
              >
                {p}
              </button>
        ...
        {filteredPosts.map((post) => (
          <div key={post.id} className="bg-card border border-border rounded-lg p-6 space-y-4">
            ...
            <div className="bg-background/50 border border-border rounded-lg p-4">
              <pre className="text-sm text-foreground whitespace-pre-wrap font-sans">{post.content}</pre>
              <div className="mt-4 pt-4 border-t border-border">
                <p className="text-sm text-accent font-medium">{post.hashtags.join(" ")}</p>
              </div>
            </div>
          </div>
        ))}
