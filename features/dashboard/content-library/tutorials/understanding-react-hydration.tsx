...
export function UnderstandingReactHydrationContent() {
  return (
    <div className="space-y-8">
      <TableOfContents
        items={[
          { id: "virtual-dom", title: "What is the Virtual DOM?", level: 2 },
          { id: "ssr", title: "Server-Side Rendering Explained", level: 2 },
          { id: "hydration", title: "What is Hydration?", level: 2 },
          { id: "errors", title: "Why Hydration Errors Happen", level: 2 },
          { id: "reading-errors", title: "Reading Error Messages", level: 2 },
          { id: "fixes", title: "Basic Fixes", level: 2 },
        ]}
      />
      ...
      <p className="text-muted-foreground leading-relaxed">
        When you build a website with plain HTML, the browser creates a tree
        structure called the DOM (Document Object Model). Changing any node
        directly is slow because the browser has to recalculate layouts and
        repaint the screen. React solves this by keeping its own lightweight
        copy in memory -- the Virtual DOM. When state changes, React updates
        this copy first, compares it with the previous version, and only updates
        the real DOM for the parts that actually changed.
      </p>
        ...
        nodes={[
          {
            id: "state",
            label: "State Change",
            description: "User clicks a button or data updates",
          },
          {
            id: "vdom",
            label: "Virtual DOM Update",
            description: "React updates its lightweight copy",
          },
          {
            id: "diff",
            label: "Reconciliation",
            description: "React compares old and new Virtual DOM",
          },
          {
            id: "patch",
            label: "DOM Patch",
            description: "Only changed elements are updated in the browser",
          },
        ]}
...
// than manipulating the actual browser DOM`}
      ...
      <InfoBox type="info" title="Why does this matter?">
        Think of the Virtual DOM as a blueprint. Comparing two blueprints is
        much cheaper than demolishing and rebuilding actual walls. React uses
        this pattern to make UI updates feel instantaneous.
      </InfoBox>
...
<!-- User sees blank screen until JS downloads and executes -->`,
  ...
  <body>
    ...
    <script src="bundle.js"></script> <!-- Still needed for interactivity -->
        ...
        features={[
          {
            ...
            description:
              "Users see content immediately instead of a blank screen",
          },
          {
            title: "Better SEO",
            description:
              "Search engines can read the fully rendered HTML directly",
          },
          {
            title: "Accessibility",
          ...
          },
          {
            ...
            description:
              "All Next.js pages use SSR by default -- no configuration needed",
          },
        ]}
        columns={2}
      />
      ...
      <p className="text-muted-foreground leading-relaxed">
        SSR sends complete HTML, but that HTML is static -- buttons do not
        respond to clicks, forms cannot submit, dropdowns will not open.
        Hydration is the process where React &quot;wakes up&quot; the
        server-rendered HTML by attaching event listeners and connecting it to
        state management. React walks through the HTML, matches it against what
        it would render, and if they match, attaches interactivity.
      </p>
...
// MISMATCH! React cannot safely attach interactivity`}
      ...
      <InfoBox type="tip" title="The glove analogy">
        Think of hydration like a hand fitting into a glove. The glove (server
        HTML) must match the hand (client React). If the shapes are different,
        it will not work.
      </InfoBox>
      ...
      <StepFlow
        steps={[
          ...
          },
          {
            ...
            title: "Random Values",
            description:
              "Math.random() or crypto.randomUUID() produce different results each time",
          },
          {
            number: "3",
            title: "Time-Dependent Code",
            description:
              "Date.now() or toLocaleTimeString() differ between server and client",
          },
          {
            number: "4",
            title: "Conditional Client State",
            description:
              "Rendering based on screen size before React can measure the screen",
          },
          {
            number: "5",
            title: "Third-Party Libraries",
            description:
              "Libraries that generate random IDs internally (e.g., Radix UI)",
          },
        ]}
      />
        ...
        terms={[
          {
            term: "- (minus) line",
            description:
              "What the SERVER generated -- this is the HTML already in the page",
          },
          {
            term: "+ (plus) line",
            description:
              "What the CLIENT expected -- this is what React would render fresh",
          },
          {
            term: "Component stack",
            description:
              "The tree of components leading to the mismatch -- start debugging from the deepest component",
          },
        ]}
      />
...
function SafeComponent() {
  ...
  return <ComplexInteractiveWidget />
      ...
      <CodeExplanation
        ...
        terms={[
          {
            term: "useState + useEffect",
            description:
              "Start with a safe server-compatible default, update to real value after hydration completes",
          },
          {
            term: "dynamic({ ssr: false })",
            description:
              "Tells Next.js to skip server rendering entirely -- the component only renders in the browser",
          },
          {
            term: "isClient check",
            description:
              "Shows a placeholder during SSR, swaps to the real component after hydration",
          },
        ]}
      />

      <KeyTakeaway>
        Hydration errors happen when server and client HTML don't match. The fix
        is always the same principle: ensure identical output during the initial
        render, then safely update to client-specific values after hydration
        completes using useEffect.
      </KeyTakeaway>
    </div>
  );
}
