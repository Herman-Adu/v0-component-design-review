...
function discoverFixes(manifest: ContentRouteManifest): FixAction[] {
  const fixes: FixAction[] = []
  const { articles, caseStudies, tutorials } = manifest

  // Slug checks — available in ContentRouteEntry
  articles.forEach((a) => {
    if (!a.slug || a.slug.trim() === "") {
      fixes.push({
        id: `article-slug-${a.slug || "empty"}`,
        title: `Article "${a.title}" has empty slug`,
        severity: "critical",
        category: "Data Integrity",
        description: "Empty slugs will cause 404 errors and broken routing.",
        affectedFiles: ["lib/strapi/dashboard/content-library/articles/"],
  ...
  caseStudies.forEach((cs) => {
    if (!cs.slug || cs.slug.trim() === "") {
      fixes.push({
        id: `casestudy-slug-${cs.slug || "empty"}`,
        title: `Case Study "${cs.title}" has empty slug`,
        severity: "critical",
        category: "Data Integrity",
        description: "Empty slugs will cause 404 errors and broken routing.",
        affectedFiles: ["lib/strapi/dashboard/content-library/case-studies/"],
  ...
  tutorials.forEach((t) => {
    if (!t.slug || t.slug.trim() === "") {
      fixes.push({
        id: `tutorial-slug-${t.slug || "empty"}`,
        title: `Tutorial "${t.title}" has empty slug`,
        severity: "critical",
        category: "Data Integrity",
        description: "Empty slugs will cause 404 errors and broken routing.",
        affectedFiles: ["lib/strapi/dashboard/content-library/tutorials/"],
  ...
  if (articleCount < 15) {
    fixes.push({
      ...
      affectedFiles: ["data/strapi-mock/dashboard/content-library/articles/"],
      suggestedFix: `Add ${15 - articleCount} more articles via Strapi admin or JSON mock files.`,
      status: "pending",
    })
  }
  ...
  if (tutorialCount < 8) {
    fixes.push({
      ...
      affectedFiles: ["data/strapi-mock/dashboard/content-library/tutorials/"],
      suggestedFix: `Add ${8 - tutorialCount} more tutorials via Strapi admin or JSON mock files.`,
      status: "pending",
    })
  }

  // Deep field validation moved to Strapi
  fixes.push({
    id: "strapi-admin-validation",
    title: "Deep content validation — use Strapi admin panel",
    severity: "info",
    category: "Migrated",
    description: "Detailed field validation (tags, steps, prerequisites, metrics, excerpts) is managed through the Strapi 5 admin interface. This tool validates routing fields only.",
    affectedFiles: ["lib/strapi/dashboard/content-library/"],
    suggestedFix: "Navigate to http://localhost:1337/admin to review and edit content quality. Use the 6-layer repository pattern for data access.",
    status: "pending",
  })
...
// ---------- Component ----------
export function FixActionsClient({ manifest }: { manifest: ContentRouteManifest }) {
  ...
  function runDiscovery() {
    setIsRunning(true)
    setTimeout(() => {
      const results = discoverFixes(manifest)
      setFixes(results)
      setHasRun(true)
      setIsRunning(false)
    }, 600)
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Scans content routing data for slug issues and coverage problems.
                Deep field validation is available via the Strapi admin panel.
              </CardDescription>
            </div>
            <Button
        ...
        <Card className="border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6 max-w-md mx-auto">
              The discovery engine checks content routing data for slug issues and coverage gaps.
              Deep field validation (tags, steps, prerequisites) is available via Strapi admin.
            </p>
      ...
      <section className="space-y-4">
        ...
        <div className="responsive-grid-2">
          {[
            ...
            { href: "/dashboard/admin/document-health/quality-engineering/pattern-compliance", label: "Pattern Compliance", desc: "Check content follows naming and structure conventions" },
          ].map((tool) => (
