...
function discoverGaps(manifest: ContentRouteManifest): Gap[] {
  const gaps: Gap[] = []

  // Build a unified content array for cross-referencing (no tags in ContentRouteEntry)
  const allContent = [
    ...manifest.articles.map((a) => ({ title: a.title, tags: [] as string[], category: a.category })),
    ...manifest.tutorials.map((t) => ({ title: t.title, tags: [] as string[], category: t.category })),
    ...manifest.caseStudies.map((cs) => ({ title: cs.title, tags: [] as string[], category: cs.category })),
    ...manifest.guides.map((g) => ({ title: g.title, tags: [] as string[], category: g.category })),
  ]
  ...
  const avgArticlesPerCat = manifest.articles.length / Math.max(Object.keys(articleCatCounts).length, 1)
  const allArticleCategories: string[] = [
  ...
  const tutorialCats = new Set(manifest.tutorials.map((t) => t.category))
  const tutorialRelevantCats: string[] = [
  ...
  const csCats = new Set(manifest.caseStudies.map((cs) => cs.category))
  const csRelevantCats: string[] = ["refactoring", "performance", "security", "architecture", "business", "cms"]
  ...
  manifest.articles.forEach((a) => {
    const lv = (a.level ?? "intermediate") as keyof typeof levelCounts
    if (lv in levelCounts) levelCounts[lv]++
  })
  const totalArticles = manifest.articles.length
  const beginnerPct = totalArticles > 0 ? (levelCounts.beginner / totalArticles) * 100 : 0
  const advancedPct = totalArticles > 0 ? (levelCounts.advanced / totalArticles) * 100 : 0
  ...
  routeTopics.forEach(({ topic, label }) => {
    const hasArticle = manifest.articles.some((a) => a.title.toLowerCase().includes(topic))
    const hasTutorial = manifest.tutorials.some((t) => t.title.toLowerCase().includes(topic))
  ...
  componentAreas.forEach(({ name, count, topic }) => {
    const hasDedicatedContent = manifest.articles.some((a) =>
      a.title.toLowerCase().includes(topic) || a.title.toLowerCase().includes(name.toLowerCase())
    )
  ...
  // Tag-based cross-referencing is now managed via Strapi admin relations.
  // Surface a count-based synergy gap if tutorials are sparse relative to articles.
  if (manifest.tutorials.length < manifest.articles.length * 0.5) {
    gaps.push({
      id: "article-tutorial-mismatch",
      title: `${manifest.tutorials.length} tutorials vs ${manifest.articles.length} articles — consider adding more tutorials`,
      type: "opportunity",
      priority: "medium",
      category: "Content Synergy",
      description: `Tutorials (${manifest.tutorials.length}) are less than half the article count (${manifest.articles.length}). Tutorials reinforce article concepts through hands-on practice.`,
      suggestedAction: `Prioritize tutorials for the most-read article topics. Use Strapi admin to manage cross-content relationships and companion content links.`,
      effort: "large",
      status: "open",
  ...
  // ---- PASS 9: REVIEW LOG KNOWLEDGE EXTRACTION ----
  ...
  // ---- PASS 10: AUDIENCE COVERAGE ANALYSIS ----
...
function computeCoverageMetrics(manifest: ContentRouteManifest): CoverageMetric[] {
  const articleCats = new Set(manifest.articles.map((a) => a.category))
  const allArticleCats = ["architecture", "security", "forms", "performance", "best-practices", "rendering", "business", "testing", "devops"]
  const tutCats = new Set(manifest.tutorials.map((t) => t.category))
  ...
  return [
    ...
    },
    {
      ...
      icon: Code,
      documented: manifest.articles.some((a) => a.title.toLowerCase().includes("component")) ? 1 : 0,
      total: 5,
...
export function GapAnalysisClient({ manifest }: { manifest: ContentRouteManifest }) {
  ...
  const coverageMetrics = useMemo(() => computeCoverageMetrics(manifest), [manifest])
  ...
  function runDiscovery() {
    setIsRunning(true)
    setTimeout(() => {
      const results = discoverGaps(manifest)
      setGaps(results)
      setHasRun(true)
      setIsRunning(false)
      ...
      <div className="responsive-grid-4">
        <Card className="border-purple-500/20">
          <CardContent className="pt-4 pb-4 text-center">
            ...
            <p className="text-2xl font-bold text-foreground">{manifest.articles.length}</p>
        ...
        <Card className="border-cyan-500/20">
          <CardContent className="pt-4 pb-4 text-center">
            ...
            <p className="text-2xl font-bold text-foreground">{manifest.caseStudies.length}</p>
        ...
        <Card className="border-green-500/20">
          <CardContent className="pt-4 pb-4 text-center">
            ...
            <p className="text-2xl font-bold text-foreground">{manifest.tutorials.length}</p>
