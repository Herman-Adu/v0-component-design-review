...
const codebaseCoverage: CodebaseArea[] = [
  ...
  },
  {
    ...
    files: [
      ...
      "contact-message-step.tsx",
      "contact-preference-step.tsx",
      "contact-review-step.tsx",
      "contact-subject-step.tsx",
    ],
  },
  {
    area: "Shared Steps",
    ...
    documentedItems: 3,
    files: [
      "shared-personal-info-step.tsx",
      "shared-address-step.tsx",
      "shared-review-step-display.tsx",
    ],
  },
  {
    area: "Animations",
  ...
  },
  {
    ...
    totalItems: 3,
    documentedItems: 3,
    files: ["use-hydration.tsx", "use-mobile.tsx", "use-toast.ts"],
  },
  {
    area: "Server Actions",
    totalItems: 4,
  ...
  },
  {
    ...
    totalItems: 4,
    documentedItems: 4,
    files: ["rate-limiter.ts", "csrf.ts", "input-sanitizer.ts", "index.ts"],
  },
  {
  ...
  },
  {
    ...
    documentedItems: 8,
    files: [
      "customer-confirmation-html.tsx",
      "business-notification-html.tsx",
      "contact-customer-html.tsx",
      "contact-business-html.tsx",
      "quotation-customer-html.tsx",
      "quotation-business-html.tsx",
      "email-styles.ts",
      "email-service.ts",
    ],
  },
  {
    area: "Content Library (6-layer Strapi)",
    totalItems: 4,
    documentedItems: 4,
    files: [
  ...
  {
    area: "Management Docs",
    totalItems: MANAGEMENT_DOC_STATS.total,
    documentedItems: MANAGEMENT_DOC_STATS.usingDocPage,
    files: MANAGEMENT_DOCS.filter((d) => d.usesDocPage).map((d) => d.slug),
  },
];
...
export function DocumentationHealthClient({
  manifest,
}: {
  ...
  const contentLibraryStructure = {
    articles: {
      ...
      byLevel: {
        beginner: manifest.articles.filter((a) => a.level === "beginner").length,
        intermediate: manifest.articles.filter((a) => a.level === "intermediate").length,
        advanced: manifest.articles.filter((a) => a.level === "advanced").length,
      },
      dynamicRoute: "/dashboard/content-library/articles/[slug]",
    },
    caseStudies: {
      total: manifest.caseStudies.length,
      byCategory: Object.fromEntries(
        [...new Set(manifest.caseStudies.map((cs) => cs.category))].map((cat) => [
          cat,
          manifest.caseStudies.filter((cs) => cs.category === cat).length,
        ]),
      ),
      dynamicRoute: "/dashboard/content-library/case-studies/[slug]",
    },
    tutorials: {
      total: manifest.tutorials.length,
      byLevel: {
        beginner: manifest.tutorials.filter((t) => t.level === "beginner").length,
        intermediate: manifest.tutorials.filter((t) => t.level === "intermediate").length,
        advanced: manifest.tutorials.filter((t) => t.level === "advanced").length,
      },
      dynamicRoute: "/dashboard/content-library/tutorials/[slug]",
    },
    guides: {
      total: manifest.guides.length,
      byCategory: Object.fromEntries(
        [...new Set(manifest.guides.map((g) => g.category))].map((cat) => [
          cat,
          manifest.guides.filter((g) => g.category === cat).length,
        ]),
      ),
  ...
  const documentationSections: DocSection[] = [
    ...
    },
    {
      ...
      lastReviewed: "2026-02-06",
      coversComponents: [
        `${STATS.frontend.components.total} components`,
        `${STATS.security.layers} security layers`,
    ...
    },
    {
      ...
      lastReviewed: "2026-02-06",
      coversComponents: [
        `${COMPONENT_COUNTS.atoms.count} Atoms`,
        `${COMPONENT_COUNTS.molecules.count} Molecules`,
        `${COMPONENT_COUNTS.organisms.count} Organisms`,
        `${COMPONENT_COUNTS.contactSteps.count} Contact Steps`,
        `${COMPONENT_COUNTS.sharedSteps.count} Shared Steps`,
        `${COMPONENT_COUNTS.animations.count} Animations`,
        `${COMPONENT_COUNTS.hooks.count} Hooks`,
        `${COMPONENT_COUNTS.articleSubComponents.count} Article sub-components`,
    ...
    },
    {
      ...
      path: "/dashboard/documentation/infrastructure-ops/testing-strategy",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: [
        "Unit testing",
    ...
    },
    {
      ...
      path: "/dashboard/documentation/infrastructure-ops/deployment-pipelines",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: [
        "CI/CD automation",
    ...
    },
    {
      ...
      path: "/dashboard/documentation/infrastructure-ops/troubleshooting",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: [
        "Common issues",
    ...
    },
    {
      ...
      coversComponents: [
        ...
        "Tutorial",
        "Case Study",
        "Field mapping from TS interfaces",
      ],
    },
    {
    ...
    {
      name: "System Vision",
      path: "/dashboard/documentation/strategic-overview/system-vision",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: [
    ...
    },
    {
      ...
      path: "/dashboard/documentation/strategic-overview/strapi-decision",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: ["CMS selection", "ROI analysis", "Enterprise features"],
    },
    {
      name: "Getting Started",
      path: "/dashboard/documentation/strategic-overview/getting-started-overview",
      status: "current",
      lastReviewed: "2026-02-06",
      coversComponents: ["Quick setup", "Core concepts", "First steps"],
    },
    {
      ...
      lastReviewed: "2026-02-06",
      coversComponents: [
        `${manifest.articles.length} Articles`,
        `${manifest.caseStudies.length} Case Studies`,
        `${manifest.tutorials.length} Tutorials`,
  ...
  const totalDocumented = codebaseCoverage.reduce(
    (acc, area) => acc + area.documentedItems,
    0,
  );
  const totalItems = codebaseCoverage.reduce(
    (acc, area) => acc + area.totalItems,
    0,
  );
  const overallCoverage = Math.round((totalDocumented / totalItems) * 100);
  const totalContent =
    contentLibraryStructure.articles.total +
    contentLibraryStructure.caseStudies.total +
    contentLibraryStructure.tutorials.total;
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Single source of truth for documentation status, coverage, and
              health
            </p>
          </div>
        </div>
      ...
      <div className="responsive-grid-5">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {totalDocumented}/{totalItems} items
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {contentLibraryStructure.articles.total}A +{" "}
              {contentLibraryStructure.caseStudies.total}CS +{" "}
              {contentLibraryStructure.tutorials.total}T
            </p>
          </CardContent>
        </Card>
        <Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Run automated checks against the documentation system to detect
            issues
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Data-driven content with dynamic [slug] routing and filter logic
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="responsive-grid-3">
            <div className="border border-border rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-1 text-sm">
                {Object.entries(contentLibraryStructure.articles.byLevel).map(
                  ([level, count]) => (
              ...
              <code className="block text-xs bg-muted p-2 rounded">
                {contentLibraryStructure.articles.dynamicRoute}
              </code>
            </div>
            ...
            <div className="border border-border rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-1 text-sm">
                {Object.entries(
                  contentLibraryStructure.caseStudies.byCategory,
                ).map(([cat, count]) => (
              ...
              <code className="block text-xs bg-muted p-2 rounded">
                {contentLibraryStructure.caseStudies.dynamicRoute}
              </code>
            </div>
            ...
            <div className="border border-border rounded-lg p-4 space-y-3">
              ...
              <div className="space-y-1 text-sm">
                {Object.entries(contentLibraryStructure.tutorials.byLevel).map(
                  ([level, count]) => (
                    <div key={level} className="flex justify-between">
                      <span
                        className={
              ...
              <code className="block text-xs bg-muted p-2 rounded">
                {contentLibraryStructure.tutorials.dynamicRoute}
              </code>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Current status of every documentation page in the system
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Documentation alignment with actual codebase files
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {area.documentedItems}/{area.totalItems}
                  </span>
                </div>
                <CoverageBar
                  documented={area.documentedItems}
                  total={area.totalItems}
                />
      ...
      <Card className="border-accent/30">
        <CardHeader>
          ...
          <CardDescription>
            All documentation stats are sourced from data/doc-manifest.ts -- the
        ...
        <CardContent className="space-y-4">
          <div className="responsive-grid-3">
            <div className="rounded-lg border border-border/50 bg-card p-4 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                The manifest imports real data arrays and computes counts via
                .length. All doc pages import STATS instead of hardcoding
                numbers.
              </p>
            </div>
            <div className="rounded-lg border border-border/50 bg-card p-4 space-y-2">
            ...
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
              <p className="text-xs text-muted-foreground">
                ...
                Rules in .v0/rules.md instruct the AI to always import from the
                manifest, never hardcode stats, and run validation after feature
                changes.
              </p>
            </div>
          </div>
