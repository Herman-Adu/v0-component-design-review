  ...
  if (!guardedItems)
    return (
      <PropGuardDiagnostic
        ...
        propName="items"
        received={items === undefined ? "undefined" : "null"}
      />
    );
        ...
        {items.map((item) => (
          <li key={item.id}>
            <a
              ...
              className={`block py-1.5 transition-all border-l-2 -ml-0.5 ${
...
export function SectionHeader({
  ...
  return (
    <h2
      id={id}
      className="text-2xl font-bold text-foreground mt-12 mb-6 flex items-baseline gap-3 scroll-mt-24"
    >
...
export function InfoBox({
  ...
  const styles = {
    ...
    tip: {
      ...
      icon: <Lightbulb className="h-5 w-5 text-green-500 shrink-0" />,
      defaultTitle: "Pro Tip",
    },
    important: {
      bg: "bg-red-500/10 border-red-500/30",
      icon: <Zap className="h-5 w-5 text-red-500 shrink-0" />,
  ...
  return (
    <div className={`${style.bg} border rounded-lg p-4 my-6`}>
      ...
      <div
        className={`text-sm text-foreground/90 leading-relaxed ${!title ? "flex gap-3" : "pl-7"}`}
      >
        {!title && style.icon}
        <div>{children}</div>
      </div>
  ...
  if (!guardedSteps)
    return (
      <PropGuardDiagnostic
        ...
        propName="steps"
        received={steps === undefined ? "undefined" : "null"}
      />
    );
...
export function ComparisonCards({
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4 my-6">
      <div
        className={`bg-card border rounded-lg p-5 ${leftIsPositive ? "border-green-500/30" : "border-red-500/30"}`}
      >
        <h4
          className={`font-semibold mb-4 ${leftIsPositive ? "text-green-500" : "text-red-500"}`}
        >
          {leftLabel}
        </h4>
      ...
      <div
        className={`bg-card border rounded-lg p-5 ${rightIsPositive ? "border-green-500/30" : "border-red-500/30"}`}
      >
        <h4
          className={`font-semibold mb-4 ${rightIsPositive ? "text-green-500" : "text-red-500"}`}
        >
          {rightLabel}
        </h4>
        <ul className="space-y-2">
...
export function BeforeAfterComparison({
          ...
          <div className="bg-card border border-green-500/20 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-green-400 font-semibold text-sm">
                {resolvedAfterTitle || "After"}
              </h4>
            </div>
            <pre className="text-xs text-muted-foreground overflow-x-auto bg-background/50 p-3 rounded">
              <code>{resolvedAfterCode}</code>
            </pre>
          </div>
        </div>
  ...
  const resolvedBeforeItems =
    beforeItems || (Array.isArray(before?.items) ? before.items : []);
  const resolvedAfterItems =
    afterItems || (Array.isArray(after?.items) ? after.items : []);
  const beforeData = {
    title: beforeTitle || before?.title || before?.label || "Before",
    items: resolvedBeforeItems,
  };
  const afterData = {
    title: afterTitle || after?.title || after?.label || "After",
    items: resolvedAfterItems,
  };
          ...
          <ul className="space-y-2">
            {beforeData.items.map((item, i) => (
              <li
                ...
                className="flex items-start gap-2 text-sm text-muted-foreground"
              >
                <Minus className="h-4 w-4 text-red-500/50 mt-0.5 shrink-0" />
                {item}
              </li>
            ))}
          </ul>
  ...
  const renderItem = (
    ...
    return (
      <div key={uniquePath} style={{ paddingLeft: `${effectiveDepth * 16}px` }}>
        <div
          className={`flex items-center gap-2 py-1 ${item.highlight ? "text-accent font-medium" : "text-muted-foreground"}`}
        >
          {item.type === "folder" ? (
        ...
        {item.children?.map((child) =>
          renderItem(child, depth + 1, uniquePath),
        )}
      </div>
    );
  };
  ...
  if (!guardedLayers)
    return (
      <PropGuardDiagnostic
        ...
        propName="layers"
        received={layers === undefined ? "undefined" : "null"}
      />
    );
  ...
  if (!guardedFeatures)
    return (
      <PropGuardDiagnostic
        ...
        propName="features"
        received={features === undefined ? "undefined" : "null"}
      />
    );
  const colClass = {
  ...
  if (!guardedMetrics)
    return (
      <PropGuardDiagnostic
        ...
        propName="metrics"
        received={metrics === undefined ? "undefined" : "null"}
      />
    );
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-6">
      {guardedMetrics.map((metric, index) => (
        ...
        >
          ...
          {metric.change && (
            <div
              className={`text-xs ${metric.positive ? "text-green-500" : "text-red-500"}`}
            >
              {metric.positive ? "+" : ""}
              {metric.change}
            </div>
...
export function DataFlowDiagram({
  ...
  if (!guardedNodes)
    return (
      <PropGuardDiagnostic
        componentName="DataFlowDiagram"
        propName="nodes"
        received={nodes === undefined ? "undefined" : "null"}
      />
    );
  return (
    <div className="bg-card border border-border rounded-lg p-6 my-6">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        {guardedNodes.map((node, index) => (
          <div key={index} className="flex items-center gap-2">
            <div className="bg-accent/10 border border-accent/30 rounded-lg px-4 py-3 text-center min-w-30">
              {node.icon && (
...
export function DecisionTree({
  ...
  const guardedDecisions = guardArrayProp(
    decisions,
    "DecisionTree",
    "decisions",
  );
  if (!guardedDecisions)
    return (
      <PropGuardDiagnostic
        ...
        propName="decisions"
        received={decisions === undefined ? "undefined" : "null"}
      />
    );
  return (
    <div className="bg-card border border-border rounded-lg p-6 my-6">
...
export function KeyTakeaway({
  ...
  return (
    <div className="bg-linear-to-r from-accent/10 to-accent/5 border border-accent/30 rounded-lg p-6 my-8">
...
export function RelatedArticles({
  ...
  const guardedArticles = guardArrayProp(
    articles,
    "RelatedArticles",
    "articles",
  );
  if (!guardedArticles)
    return (
      <PropGuardDiagnostic
        ...
        propName="articles"
        received={articles === undefined ? "undefined" : "null"}
      />
    );
  return (
        ...
        {guardedArticles.map((article, index) => (
          <Link
            ...
            href={
              article.href ||
              (article.slug
                ? `/dashboard/content-library/articles/${article.slug}`
                : "#")
            }
...
export function StatsTable({
  ...
  if (!guardedHeaders)
    return (
      <PropGuardDiagnostic
        componentName="StatsTable"
        propName="headers"
        received={headers === undefined ? "undefined" : "null"}
      />
    );
  if (!guardedRows)
    return (
      <PropGuardDiagnostic
        componentName="StatsTable"
        propName="rows"
        received={rows === undefined ? "undefined" : "null"}
      />
    );
  return (
    <div className="my-6">
      ...
      <ol className="space-y-4">
        {normalizedItems.map((item, index) => (
          <li key={index} className="flex gap-4">
            <span className="shrink-0 w-8 h-8 rounded-full bg-accent/20 text-accent font-bold flex items-center justify-center text-sm">
              {index + 1}
            </span>
            <div className="flex-1 pt-1">
...
export function ProcessFlow({
  ...
  const normalizedSteps = guardedSteps.map((step) => {
    if (typeof step === "string")
      return { label: step, sublabel: undefined, color: undefined };
    return {
        ...
        <div className="flex items-center justify-between gap-2 overflow-x-auto">
          {normalizedSteps.map((step, index) => (
            <div key={index} className="flex items-center">
              <div
                className={`px-4 py-3 rounded-lg text-center min-w-30 ${
                  step.color === "green"
          ...
          <div>
            ...
            <div className="space-y-2">
              {leftItems.map((item, index) => (
                <div
                  key={index}
                  className={`px-4 py-2 rounded border ${getItemStyle(item.type)}`}
                >
                  {item.label}
                </div>
              ))}
    ...
    <div className="bg-card border border-border rounded-lg p-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          {change && (
            <p className="text-sm text-accent font-medium mt-1">{change}</p>
          )}
          {description && (
        ...
        {Icon && (
          <div
            className={`p-2 rounded-lg ${trend === "up" ? "bg-green-500/20" : trend === "down" ? "bg-red-500/20" : "bg-accent/20"}`}
          >
            <Icon
              className={`h-5 w-5 ${trend === "up" ? "text-green-400" : trend === "down" ? "text-red-400" : "text-accent"}`}
            />
          </div>
        )}
      </div>
      {trend && (
        <div
          className={`flex items-center gap-1 mt-2 text-xs ${trend === "up" ? "text-green-400" : trend === "down" ? "text-red-400" : "text-muted-foreground"}`}
        >
          {trend === "up" ? (
